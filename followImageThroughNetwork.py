...
def block_save_color(A):
  ...
  for i in range(m):
    for j in range(n):
      ax.add_patch(patches.Rectangle((j/m, i/m), 1/m, 1/n, facecolor=(A[0,m-i-1,j].item(), A[1,m-i-1,j].item(), A[2,m-i-1,j].item())))
  ax.set_aspect('equal')
  plt.axis('off')
  plt.draw()
  plt.savefig('presentation/followImageThroughNetwork/%04d.jpg' % count, bbox_inches='tight')
  count += 1
...
def block_save(A):
  ...
  for i in range(m):
    for j in range(n):
      ax.add_patch(patches.Rectangle((j/m, i/m), 1/m, 1/n, facecolor=(A[m-i-1,j].item(), A[m-i-1,j].item(), A[m-i-1,j].item())))
  ax.set_aspect('equal')
  plt.axis('off')
  plt.draw()
  plt.savefig('presentation/followImageThroughNetwork/%04d.jpg' % count, bbox_inches='tight')
...
#np_image = plt.imread('images/nina/001013.jpg')
np_image = plt.imread('images/franz/001010.jpg')
